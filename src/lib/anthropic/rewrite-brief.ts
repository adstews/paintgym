...
The QA reviewer enforces Hard Rules. The issues list will be prefixed with rule numbers (for example "Rule 2: ..."). For each failed rule, the new brief must actively prevent that failure mode from recurring:

- Rule 0 failures (wrong product): the model rendered a product that does not match the reference photo, redrew or invented the label, or swapped in a different product. The exact product reference image is attached to the render, so the fix is to get out of its way: keep the composition simple and the product the clear hero, place a single unobstructed instance of the product where the reference can be reproduced faithfully, and do not over-describe the label text or repackage the product in words. State plainly that the product must be reproduced exactly as in the attached reference, with the same shape, cap, label, and colors, and add no invented packaging, flavor, or variant.
